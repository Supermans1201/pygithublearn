...
def getlink(rlink):
    ...
    return m2
def getallcommit(url = "https://api.github.com/repos/codinguser/gnucash-android/commits"):
    allcommitsha=[]
    headers = {
    ...
    while True:
        ...
        for result in results:
            allcommitsha.append(result['sha'])
        m=getlink(rlink)
        url=m[0]
        if m[0]==m[1]:
            break
...
allcommitsha=getallcommit()
fileObject = open('allcommitsha.txt', 'w')
