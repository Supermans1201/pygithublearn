...
class Yuchuli:
    ...
    def displayCount(self):
        print "Total Paqu %d" % Yuchuli.yuchuliCount
    ...
    def getsmailarword(self, word="activity", type="all", number=10):
        ...
        if self.type == "All" or self.type == "all":
            print "类型是所有..."
            self.sourcefilename = self.diffmodelfilename

        elif self.type == "Java" or self.type == "java":
            print "类型是Java..."
            self.sourcefilename = self.diffmodeljavafilename

        elif self.type == "Xml" or self.type == "xml":
            print "类型是Xml..."
            self.sourcefilename = self.diffmodelxmlfilename
    ...
    def genmodel(self,type="all"):
        ...
        if self.type == "All" or self.type == "all":
            print "类型是所有..."
            self.targetfilename = self.diffmodelfilename
            self.sourcefilename = self.diffpredealfilename

        elif self.type == "Java" or self.type == "java":
            print "类型是Java..."
            self.targetfilename = self.diffmodeljavafilename
            self.sourcefilename = self.diffpredealjavafilename
        elif self.type == "Xml" or self.type == "xml":
            print "类型是Xml..."
            self.targetfilename = self.diffmodelxmlfilename
            self.sourcefilename = self.diffpredealxmlfilename
    ...
    def gendirectory(self,type="all"):
        ...
        if self.type == "All" or self.type == "all":
            print "类型是所有..."
            self.targetfilename = self.directoryfilename
            self.sourcefilename = self.diffpredealfilename

        elif self.type == "Java" or self.type == "java":
            print "类型是Java..."
            self.targetfilename = self.directoryjavafilename
            self.sourcefilename = self.diffpredealjavafilename
        elif self.type == "Xml" or self.type == "xml":
            print "类型是Xml..."
            self.targetfilename = self.directoryxmlfilename
            self.sourcefilename = self.diffpredealxmlfilename
        print self.sourcefilename, "->", self.targetfilename
    ...
    def gendiffpredealtxt(self,type="all",stragy=0):
        ...
        if self.type == "All" or self.type == "all":
            print "类型是所有..."
            self.targetfilename = self.diffpredealfilename
            self.sourcefilename = self.difffilename

        elif self.type == "Java" or self.type == "java":
            print "类型是Java..."
            self.targetfilename = self.diffpredealjavafilename
            self.sourcefilename = self.diffjavafilename
        elif self.type == "Xml" or self.type == "xml":
            print "类型是Xml..."
            self.targetfilename = self.diffpredealxmlfilename
            self.sourcefilename = self.diffxmlfilename
        print self.sourcefilename,"->",self.targetfilename
    ...
    def gendiffsanitizationtxt(self,type="all"):
        ...
        if self.type== "All" or self.type== "all":
            print "类型是所有..."
            self.targetfilename=self.difffilename
        elif  self.type== "Java" or self.type== "java":
            print "类型是Java..."
            self.targetfilename = self.diffjavafilename
        elif  self.type== "Xml" or self.type== "xml":
            print "类型是Xml..."
            self.targetfilename=self.diffxmlfilename

        if os.path.exists(self.targetfilename):
            os.remove(self.targetfilename)
        self.difftextfile = open(self.targetfilename, 'a')

        self.sourcefilename=self.rootdir
